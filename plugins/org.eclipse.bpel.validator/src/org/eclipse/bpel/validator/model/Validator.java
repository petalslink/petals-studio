...
public class Validator implements IConstants {
	...
	final public void validate ( String tag ) {
		...
		try {
			...
			this.fRuleRunner.runRules ( tag );
			if (tag.equals(PASS2)) {
				end();
			}
		} catch (Throwable t) {
			this.mLogger.logp(Level.SEVERE, getClass().getName(),
					"validate",
	...
	public boolean hasProblems () {
		return this.fProblems.isEmpty() == false;
	}
	...
	public void adopt ( IProblem problem , INode node ) {
		...
		problem.setAttribute(IProblem.LINE_NUMBER,
				this.mModelQuery.lookup(node, IModelQueryLookups.LOOKUP_NUMBER_LINE_NO,-1));

		problem.setAttribute(IProblem.COLUMN_NUMBER,
				this.mModelQuery.lookup(node, IModelQueryLookups.LOOKUP_NUMBER_COLUMN_NO,-1));

		problem.setAttribute(IProblem.CHAR_END,
				this.mModelQuery.lookup(node, IModelQueryLookups.LOOKUP_NUMBER_CHAR_END,-1));

		problem.setAttribute(IProblem.CHAR_START,
				this.mModelQuery.lookup(node, IModelQueryLookups.LOOKUP_NUMBER_CHAR_START,-1));

		problem.setAttribute(IProblem.LOCATION,
				this.mModelQuery.lookup(node, IModelQueryLookups.LOOKUP_TEXT_LOCATION,null,null));

		problem.setAttribute(IProblem.ADDRESS_MODEL,
				this.mModelQuery.lookup(node, IModelQueryLookups.LOOKUP_TEXT_HREF,null,null));

		problem.setAttribute(IProblem.ADDRESS_XPATH,
				this.mModelQuery.lookup(node, IModelQueryLookups.LOOKUP_TEXT_HREF_XPATH,null,null));
	...
	protected IProblem createProblem ( INode node ) {
		...
		Rule r = this.fRuleRunner.getExecutingRule();
	...
	protected String toString (QName qname) {
		...
		if (isEmptyOrWhitespace(prefix)) {
			prefix = this.mModelQuery.lookup(this.mNode, IModelQueryLookups.LOOKUP_TEXT_NS2PREFIX, qname.getNamespaceURI(), null );
		}
