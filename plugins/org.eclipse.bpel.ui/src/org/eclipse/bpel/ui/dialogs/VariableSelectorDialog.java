...
public class VariableSelectorDialog extends SelectionAndCreationDialog {
	 ...
	 */
	protected class VariableComparator implements Comparator<String> {
		/**
		...
		public int compare( String name1, String name2 ) {
			...
			if (VariableSelectorDialog.this.variableType != null) {
				...
				if (VariableSelectorDialog.this.variableType.equals(type1) && !VariableSelectorDialog.this.variableType.equals(type2)) return -1;
				if (!VariableSelectorDialog.this.variableType.equals(type1) && VariableSelectorDialog.this.variableType.equals(type2)) return 1;
	...
	protected Map<String,Variable> nameToVariable;
	...
	protected void handleNewButtonPushed() {
		...
		Process process = ModelHelper.getProcess(this.modelObject);
		...
		if (nameDialog.open() == Window.OK) {
			...
			command.add(new SetVariableTypeCommand(variable, this.variableType));
	...
	protected Variable getVariable(String name) {
		Variable result = this.nameToVariable.get(name);
		if (result != null) return result;
		for( Variable variable : this.variables ) {
			if (name.equals(variable.getName())) {
				this.nameToVariable.put(name, variable);
				return variable;
			}
		}
