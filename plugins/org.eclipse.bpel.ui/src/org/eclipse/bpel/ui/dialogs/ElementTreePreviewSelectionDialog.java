...
public class ElementTreePreviewSelectionDialog extends ElementTreeSelectionDialog {
	...
	public class TreeFilter extends ViewerFilter {
		...
		public boolean select(Viewer viewer, Object parentElement, Object element) {
			if (element instanceof BPELVariableTreeNode) {
				...
				if (var.getName().indexOf(ElementTreePreviewSelectionDialog.this.theFilter) > -1)
					return true;
			}
				...
				if (xsdelem.getName().indexOf(ElementTreePreviewSelectionDialog.this.theFilter) > -1)
					return true;
			}
				...
				if (xsdattribute.getName().indexOf(ElementTreePreviewSelectionDialog.this.theFilter) > -1)
					return true;
			}
			else if (element instanceof PartTreeNode) {
				Part part = (Part)((PartTreeNode)element).getModelObject();
				if (part.getName().indexOf(ElementTreePreviewSelectionDialog.this.theFilter) > -1)
					return true;
			}
	...
	protected Control createDialogArea(Composite parent) {
		...
		this.filterLabel = new Label(filter, SWT.NONE);
		this.filterLabel.setText(XPathExpressionUtil.LBL_FILTER_TEXT);
		data1 = new GridData();
		data1.widthHint = 80;
		this.filterLabel.setLayoutData(data1);

		this.filterText = new Text(filter, SWT.BORDER);
		data1 = new GridData(GridData.FILL_HORIZONTAL);
		//data.widthHint = 300;
		this.filterText.setLayoutData(data1);
		...
		this.previewLabel = new Label(previewMain, SWT.NONE);
		this.previewLabel.setText(XPathExpressionUtil.LBL_PREVIEW_TEXT);
		data = new GridData();
		data.widthHint = 80;
		this.previewLabel.setLayoutData(data);

		this.previewText = new Text(previewMain, SWT.BORDER);
