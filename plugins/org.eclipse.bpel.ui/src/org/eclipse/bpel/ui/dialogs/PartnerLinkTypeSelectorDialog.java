...
public class PartnerLinkTypeSelectorDialog extends BrowseSelectorDialog {
	...
	public PartnerLinkTypeSelectorDialog (Shell parent, EObject eObj ) {
		...
		this.resourceContentProvider = new WSDLDefinitionFromResourceContentProvider(
					this.modelObject.eResource().getResourceSet());
	...
	/**
	 * Handle the check button and radio button call-backs.
	 * 
	 * @param id
	 * @param checked
	...
	protected void okPressed() {
		...
		if (obj instanceof PortType) {
			...
			if( dialog.open() == Window.OK ) {
				...
				if( this.fPartnerLinkType != null
							&& checkNamespace( this.fPartnerLinkType ))
					super.okPressed();
			}
	...
	protected void createBrowseFilterGroupButtons ( Group  group ) {
		...
		this.fShowPortTypes = createCheckButton(
					group,
					Messages.PartnerLinkTypeSelectorDialog_5,
					BID_SHOW_PORT_TYPES,
					this.showPortTypes  );
