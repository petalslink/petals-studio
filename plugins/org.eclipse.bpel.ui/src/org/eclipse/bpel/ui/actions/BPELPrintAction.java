...
public class BPELPrintAction extends PrintAction {
	...
	protected static class InternalPrintGraphicalViewerOperation extends PrintGraphicalViewerOperation {
		...
		protected void setupPrinterGraphicsFor(Graphics graphics, IFigure figure) {
			...
			double dpiScale = ((double) getPrinter().getDPI().x) / ((double) Display.getCurrent().getDPI().x);
		...
		protected void printPages() {
			...
			for (int j=0; j < this.fPagesY; j++) {
				for (int i=0; i < this.fPagesX; i++) {
