...
public class CreatePartnerLinkTypeWizardNamePage extends WizardPage {
	...
	protected CreatePartnerLinkTypeWizardNamePage(String pageName) {
		...
		setImageDescriptor(BPELUIPlugin.getDefault().getImageDescriptor(
	...
	Composite createPartnerLinkName(Composite parent) {
		...
		this.partnerLinkTypeName = new Text(fields, SWT.BORDER);
		GridData data = new GridData(GridData.FILL_HORIZONTAL);
		data.widthHint = SIZING_TEXT_FIELD_WIDTH;
		this.partnerLinkTypeName.setLayoutData(data);
		this.partnerLinkTypeName.setFont(parent.getFont());

		this.partnerLinkTypeName.addListener(SWT.Modify, new Listener() {
			public void handleEvent(Event event) {
				setPageComplete(validatePage());
			}
		});

		this.hintText = new Text(parent, SWT.READ_ONLY | SWT.WRAP | SWT.SCROLL_LINE);
		data = new GridData(GridData.FILL_HORIZONTAL);
		data.widthHint = SIZING_TEXT_FIELD_WIDTH;
		data.heightHint = 80;
		this.hintText.setLayoutData(data);
		this.hintText.setFont(parent.getFont());
	...
	protected boolean validatePage() {
		...
		String pltName = this.partnerLinkTypeName.getText();
		...
		if (this.fDefinitions != null) {
			...
			Iterator it = this.fDefinitions.getEExtensibilityElements().iterator();
			while (it.hasNext()) {
	...
	public String getPartnerLinkTypeName () {
		return this.partnerLinkTypeName.getText();
	}
