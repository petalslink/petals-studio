...
public class AddImportCommand extends AutoUndoCommand {
	...
	public AddImportCommand ( Process process , Object object, ResourceInfo info ) {
		...
		this.fImport = createImport( object );
	...
	Import createImportFrom( Definition defn ) {
		...
		// Create the import and the basic properties
		Import imp = BPELFactory.eINSTANCE.createImport();
		...
		// Set the location
		Resource bpelResource = this.fProcess.eResource();
		java.net.URI bpelUri = EmfUtils.convertEmfUriToJavaNetUri( bpelResource.getURI());
		java.net.URI wsdlUri = null;
		URI schemaURI = null;

		if( defn.getLocation() == null ) {
			if( this.fResourceInfo != null )
				wsdlUri =  this.fResourceInfo.getFile().getLocation().toFile().toURI();
		} else {
			schemaURI = URI.createURI( defn.getLocation());
			wsdlUri = EmfUtils.convertEmfUriToJavaNetUri( schemaURI );
		}

		// Resolve the relative location
		String relativeLocation = null;
		try {
			if( bpelUri != null && wsdlUri != null )
				relativeLocation = UriUtils.getRelativeLocationToUri( bpelUri, wsdlUri ).toString();
			else if( wsdlUri != null )
				relativeLocation = wsdlUri.toString();
			else if( schemaURI != null )
				relativeLocation = schemaURI.toString();

		} catch( Exception e ) {
			BPELUIPlugin.log( e, IStatus.ERROR );
		}

		imp.setLocation ( relativeLocation );
		return imp;
	}
}
