...
public abstract class AutoUndoCommand extends AbstractEditModelCommand {
	private final Set<Object> fModelRoots;
	...
	public AutoUndoCommand(String label, EObject modelRoot) {
		...
		this.fModelRoots = Collections.singleton((Object) modelRoot);
	}
	...
	private Object calculateLeastCommonAncestor() {
		...
		if (roots.length == 1
				&& roots[ 0 ] instanceof WSDLElement )
		...
		while (true) {
			...
			if (current.isEmpty()) {
				Resource res = ((WSDLElement)roots[0]).eResource();
				if( res instanceof BPELResource )
					return ((BPELResource) res).getProcess().getElement();
			}
		}
	}
