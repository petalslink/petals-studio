...
public class NamespaceUtils
{
	...
	protected static HashMap<String,String> fCharToEscaped = new HashMap<String,String>( 15 );
	static
	{
	...
	public static String convertUriToNamespace(String uri)
	{
		...
		Byte hexByte = null;
		ArrayList<Byte> contiguousBytes = new ArrayList<Byte>();

		for (int i=uri.indexOf(HEX_PREFIX); i>=0; i=uri.indexOf(HEX_PREFIX, prevIndex))
		{
			// get hex byte
			hexByte = getHexByte(uri, i);
			contiguousBytes.clear();
			contiguousIndex = i;
			StringBuffer hexChunk = new StringBuffer();
			...
			while (hexByte!=null)
			{
				...
				hexChunk.append( uri.substring(contiguousIndex, contiguousIndex+3));
				prevIndex = contiguousIndex+1; // +1 for %
			...
			for (int j=0; j<cbytes.length; j++)
				cbytes[j] = (contiguousBytes.get(j)).byteValue();
			...
			try
			{
				...
				int beginIndex = namespace.indexOf( hexChunk.toString());
	...
	public static String escapeUnsafeCharacters(String anyURI, boolean escapePercent)
	{
		...
		if (escapePercent)
			anyURI = anyURI.replaceAll("\\"+PERCENT, fCharToEscaped.get(PERCENT)); //$NON-NLS-1$

		String key = null;

		// escape all other characters except %
		for( Object element : fCharToEscaped.keySet() ) {
			key = (String)element;
			if (key.equals(PERCENT))
				continue;

			anyURI = anyURI.replaceAll("\\"+key, fCharToEscaped.get(key)); //$NON-NLS-1$
		}
