...
public class ModelHelper {
	...
	public static Boolean getSuppressJoinFailure(Object context)  {
		return isSetSuppressJoinFailure(context)? getSuppressJoinFailure2(context) : false;
	}
	...
	static public EObject getImportedDocument ( Import imp ) {
		...
		for( ImportResolver resolver : resolvers ) {
			EObject result = resolver.resolve(imp, null, null, null );
			if (result != null) {
				return result;
			}
		}
	...
	{
		ArrayList<Object> al = new ArrayList<Object>( 8 );
		...
		while ( it.hasNext() )
		{
			...
			ImportResolver[] resolvers = ImportResolverRegistry.INSTANCE.getResolvers(imp.getImportType());
			for( ImportResolver resolver : resolvers ) {
				al.addAll( resolver.resolve (imp, ImportResolver.RESOLVE_SCHEMA ) );
			}
			// next import
		}
	...
	static public List<Definition> getDefinitions ( Object context ) {
		...
		while ( it.hasNext()) {
			...
			ImportResolver[] resolvers = ImportResolverRegistry.INSTANCE.getResolvers(imp.getImportType());
			for( ImportResolver resolver : resolvers ) {
				Object o = resolver.resolve( imp, ImportResolver.RESOLVE_DEFINITION );
				al.addAll((Collection<? extends Definition>) o);
			}
			// next import
		}
	...
	/**
	 * Compares two messages based on their QName.
	 * @param message1 the first message
	 * @param message2 the second message
	 * @return true if the messages have the same QName, false otherwise
	 * 
	 * FIXME: should equality be computed based on structure?
	 */
	public static boolean areEqual( Message message1, Message message2 ) {
		...
		if( message1 == message2 ) {
			...
			if( message1.getQName() != null )
				equal = message1.getQName().equals( message2.getQName());
			else
				equal = message2.getQName() == null;
		}
	...
	/**
	 * Compares two XSD elements based on their QName.
	 * @param decl1 the first element
	 * @param decl2 the second element
	 * @return true if the messages have the same QName, false otherwise
	 * 
	 * FIXME: should equality be computed based on structure?
	 */
	public static boolean areEqual( XSDElementDeclaration decl1, XSDElementDeclaration decl2 ) {
		...
		if( decl1 == decl2 ) {
			...
			if( decl1.getQName() != null )
				equal = decl1.getQName().equals( decl2.getQName());
			else
				equal = decl2.getQName() == null;
		}
	...
	/**
	 * Compares two XSD elements based on their QName.
	 * @param type1 the first element
	 * @param type2 the second element
	 * @return true if the messages have the same QName, false otherwise
	 * 
	 * FIXME: should equality be computed based on structure?
	 */
	public static boolean areEqual( XSDTypeDefinition type1, XSDTypeDefinition type2 ) {
		...
		if( type1 == type2 ) {
			...
			if( type1.getQName() != null )
				equal = type1.getQName().equals( type2.getQName());
			else
				equal = type2.getQName() == null;
		}
