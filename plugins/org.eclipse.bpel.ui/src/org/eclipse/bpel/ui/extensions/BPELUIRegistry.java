...
public class BPELUIRegistry {
	...
	static final String EXTPT_EDIT_POLICY_PROVIDER = "editPolicyProviders"; //$NON-NLS-1$
	static final String ELEMENT_EDIT_POLICY_PROVIDER = "provider"; //$NON-NLS-1$
	...
	private IEditPolicyProvider[] editPolicyProviders;
		...
		for(IConfigurationElement editor : getConfigurationElements(EXTPT_EXPRESSION_EDITORS) ) {
			...
			if (editor.getName().equals(ELEMENT_EDITOR)) {
				...
				if (language == null || clazz == null) {
					String pluginId = BPELUIPlugin.getDefault().getBundle().getSymbolicName();
					...
					BPELUIPlugin.getDefault().getLog().log(status);
				} else {
	...
	private void readUIObjecFactories() {
		...
		List<UIObjectFactoryDescriptor> factories = new ArrayList<UIObjectFactoryDescriptor> ();
	...
	/**
	 * Read the edit policy provided by extensions
	 */
	private void readEditPolicyProviders() {
		List<IEditPolicyProvider> editPolicyProviders = new ArrayList<IEditPolicyProvider>();

		for (IConfigurationElement element : getConfigurationElements(EXTPT_EDIT_POLICY_PROVIDER)) {
			if (element.getName().equals(ELEMENT_EDIT_POLICY_PROVIDER)) {
				try {
					IEditPolicyProvider provider = (IEditPolicyProvider)element.createExecutableExtension(ATT_CLASS);
					editPolicyProviders.add(provider);
				} catch (CoreException e) {
					BPELUIPlugin.log(e);
				}
			}
		}
		this.editPolicyProviders = editPolicyProviders.toArray(new IEditPolicyProvider[editPolicyProviders.size()]);
	}

	/**
	 * @return the contributed editPolicy providers from extensions
	 */
	public IEditPolicyProvider[] getEditPolicyProviders() {
		return this.editPolicyProviders;
	}
}
