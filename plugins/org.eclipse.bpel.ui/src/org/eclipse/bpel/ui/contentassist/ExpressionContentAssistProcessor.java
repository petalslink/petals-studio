...
implements IContentAssistProcessor, ICompletionListener {
	...
	static class ProposalType {
		private final int theType;
		private final String theBeginsWith;
		...
		boolean isVariable() { return ((this.theType & PROPTYPE_VARIABLE) == PROPTYPE_VARIABLE); }
		boolean isFunction() { return ((this.theType & PROPTYPE_FUNCTION) == PROPTYPE_FUNCTION); }
		boolean isOperator() { return ((this.theType & PROPTYPE_OPERATOR) == PROPTYPE_OPERATOR); }
		boolean isVariableAndFunction() { return ((this.theType & (PROPTYPE_VARIABLE | PROPTYPE_FUNCTION)) == (PROPTYPE_VARIABLE | PROPTYPE_FUNCTION)); }
	}

	// helper class for defining expression types within xpath expression... used by XPathStack
	static class ExpressionType {
		private final int theType;
		private final String theGrammar;
	...
	protected class XPathStack {
		...
		XPathStack(ITextViewer viewer, int offset) {
			this.theCallStack = new Stack<ExpressionType>();
			this.theStatus = 0;	// flag for stack's integrity
		...
		private boolean parseNumberExpression() throws BadLocationException {
			...
			while (this.theIndex < this.theOffset) {
				...
				// variable
				if (currChar == '$') {
					if ((this.theTopOfStackExprType == ExpressionType.EXPRTYPE_UNARY_OPERATOR) ||
								//(theTopOfStackExprType == ExpressionType.EXPRTYPE_BOOLEAN_OPERATOR) ||
								(this.theTopOfStackExprType == ExpressionType.EXPRTYPE_NUMERIC_OPERATOR))
				...
				// literal
					...
					if ((this.theTopOfStackExprType == ExpressionType.EXPRTYPE_BOOLEAN_OPERATOR) ||
								(this.theTopOfStackExprType == ExpressionType.EXPRTYPE_NUMERIC_OPERATOR) ||
								(this.theTopOfStackExprType == ExpressionType.EXPRTYPE_UNARY_OPERATOR) ||
								(this.theTopOfStackExprType == -1)) {
						// check to see if it's a unary operator
						if (currChar == '-') {
							this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_UNARY_OPERATOR, MINUS));
							this.theTopOfStackExprType = ExpressionType.EXPRTYPE_UNARY_OPERATOR;
							this.theIndex++;
							if (parseNumberExpression()) {
								...
								this.theCallStack.pop();
								// push "numeric" value
								this.theCallStack.push(new ExpressionType(ExpressionType.CLASS_NUMERIC, EMPTY_STRING));
								this.theTopOfStackExprType = ExpressionType.CLASS_NUMERIC;
							}
						}
						else {
							this.theStatus = -1;
						...
						if ((this.theIndex+1) < this.theOffset) {
							if (isReservedOperatorCharacter(this.theDocument.getChar(this.theIndex+1))) {
								this.theIndex++;
								tempOper = tempOper + Character.toString(this.theDocument.getChar(this.theIndex));
							}
							int tempType = 0;
					...
					this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_NEW_EXPRESSION, Character.toString(currChar)));
					this.theTopOfStackExprType = ExpressionType.EXPRTYPE_NEW_EXPRESSION;
					this.theIndex++;
					if (parseBooleanExpression()) {
						...
						this.theCallStack.push(new ExpressionType(ExpressionType.CLASS_EXPRESSION, EMPTY_STRING));
						this.theTopOfStackExprType = ExpressionType.CLASS_EXPRESSION;
						if ((type == ExpressionType.EXPRTYPE_UNARY_OPERATOR) ||
									(this.theTopOfStackExprType == ExpressionType.EXPRTYPE_BOOLEAN_OPERATOR) ||
									(this.theTopOfStackExprType == ExpressionType.EXPRTYPE_NUMERIC_OPERATOR))
							return true;
					}
					else
					...
					int type = this.theTopOfStackExprType;
					proceed = parseWord();
					if (proceed) {
						if ((this.theTopOfStackExprType == ExpressionType.EXPRTYPE_NUMERIC_OPERATOR) ||
									(this.theTopOfStackExprType == ExpressionType.EXPRTYPE_BOOLEAN_OPERATOR)) {
			...
			while (this.theIndex < this.theOffset) {
				...
				if (nextChar == '(') {
					if( sb.toString().length() > 0) {
						this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_FUNCTION, sb.toString()));
						this.theTopOfStackExprType = ExpressionType.EXPRTYPE_FUNCTION;
				...
				if (Character.isWhitespace(nextChar)) {
					...
					if (word.length() > 0) {
						if ((word.compareToIgnoreCase(MOD) == 0) ||
							...
							this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_NUMERIC_OPERATOR, word));
							this.theTopOfStackExprType = ExpressionType.EXPRTYPE_NUMERIC_OPERATOR;
							return true;
						}
						if ((word.compareToIgnoreCase(AND) == 0) ||
									(word.compareToIgnoreCase(OR) == 0)) {
							this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_BOOLEAN_OPERATOR, word));
							this.theTopOfStackExprType = ExpressionType.EXPRTYPE_BOOLEAN_OPERATOR;
							return true;
						}
						...
						while (tempIndex < this.theOffset) {
							if (!Character.isWhitespace(this.theDocument.getChar(tempIndex))) {
								if (this.theDocument.getChar(tempIndex) == '(') {
									break;
								}
								this.theStatus = -1;
								return false;
		...
		private boolean parseFunction() throws BadLocationException {
			...
			while (this.theIndex < this.theOffset) {
				nextChar = this.theDocument.getChar(this.theIndex);
				if (nextChar == '(')
					this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_FUNCTION_ARGUMENTS, OPEN_PAREN));
				else if (nextChar == ')') {
					try {
						ExpressionType tempExpr = this.theCallStack.peek();
					...
					this.theIndex++;
					this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_FUNCTION_ARGUMENT_SEPARATOR, COMMA));
		...
		private boolean parseLiteral() throws BadLocationException {
			...
			while (this.theIndex < this.theOffset) {
				...
				if (nextChar != delimiter)
					...
					sb.append( nextChar );
					this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_LITERAL, sb.toString()));
					this.theTopOfStackExprType = ExpressionType.EXPRTYPE_LITERAL;
					return true;
				}
				this.theIndex++;
			}
		...
		private boolean parseNumber() throws BadLocationException {
			...
			while (this.theIndex < this.theOffset) {
				...
				if ((Character.isDigit(nextChar) || (nextChar == '.')))
					...
					this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_NUMBER, sb.toString()));
					this.theTopOfStackExprType = ExpressionType.EXPRTYPE_NUMBER;
					this.theIndex--;
					return true;
				}
				this.theIndex++;
		...
		private boolean parseVariable() throws BadLocationException {
			...
			this.theIndex++; // move past $ token
			StringBuffer sb = new StringBuffer( DOLLAR );
			char nextChar;
			while (this.theIndex < this.theOffset) {
				nextChar = this.theDocument.getChar(this.theIndex);
				if (isLocationPathCharacter(nextChar))
					...
					this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_VARIABLE, sb.toString()));
					this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_NEW_EXPRESSION, OPEN_BRACKET));
					this.theIndex++;
					boolean proceed = parseBooleanExpression();
					if (proceed) {
						nextChar = this.theDocument.getChar(this.theIndex);
					...
					this.theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_VARIABLE, sb.toString()));
					this.theTopOfStackExprType = ExpressionType.EXPRTYPE_VARIABLE;
					this.theIndex--;
					return true;
				}
				this.theIndex++;
	...
	private ICompletionProposal[] generateProposals(ITextViewer viewer, int offset) {
		...
		case 0:
			this.theContentAssistant.setStatusMessage(Messages.getString("ExpressionContentAssistProcessor.44")); //$NON-NLS-1$
		...
		case 1:
			this.theContentAssistant.setStatusMessage(Messages.getString("ExpressionContentAssistProcessor.45"));   //$NON-NLS-1$
			return generateFunctionProposals(viewer, funcStart, offset);
		case 2:
			this.theContentAssistant.setStatusMessage(Messages.getString("ExpressionContentAssistProcessor.46"));   //$NON-NLS-1$
			return generateOperatorProposals(EMPTY_STRING, offset);

		case 3:
			if (this.theExpressionContext.compareTo(IEditorConstants.ET_JOIN) == 0)
				this.theContentAssistant.setStatusMessage(Messages.getString("ExpressionContentAssistProcessor.48")); //$NON-NLS-1$
			else
				this.theContentAssistant.setStatusMessage(Messages.getString("ExpressionContentAssistProcessor.49")); //$NON-NLS-1$
	...
	ICompletionProposal[] generateOperatorProposals(String context, int offset) {
		...
		Image img = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_OPERATION_16);
	...
	ICompletionProposal[] generateLinkProposals(String context, int offset) {
		...
		Image linkImg = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_LINK_16);

		for( Object next : new LinkContentProvider( LinkContentProvider.INCOMING ).getElements(this.theModel) ) {
			Link link = (Link) next;
	...
	ICompletionProposal[] generateVariableProposals(String context, int offset) {
		...
		Image varImg = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_VARIABLE_16);
		Image partImg = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_PART_16);
		Image elementImg = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_XSD_ELEMENT_DECLARATION_16);
		Image attrImg = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_XSD_ATTRIBUTE_DECLARATION_16);
		if (seekChildren) {
			// walk down path
			int index = 0;
			int level = 0;
				...
				if ((t == '.') || (t == '/')) {
					...
					if (level == 0) {
						for( Variable variable : variables ) {
							if (levelName.compareTo(variable.getName()) == 0) {
								currVar = variable;
