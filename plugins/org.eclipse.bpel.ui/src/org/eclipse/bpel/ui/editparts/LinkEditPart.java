...
public class LinkEditPart extends AbstractConnectionEditPart {
	...
	private final MultiObjectAdapter adapter;
	...
	protected void addAllAdapters() {
		...
		if (link.getSources().size() > 0) {
			...
			if (source != null) {
				this.adapter.addToObject(source);
				sourceActivity = source.getActivity();
			}

			if (sourceActivity != null) {
				this.adapter.addToObject(sourceActivity);

				EObject sourceParent = sourceActivity.eContainer();
				if (sourceParent != null) {
					this.adapter.addToObject(sourceParent);
				}
			}
		}
		if (link.getTargets().size() > 0) {
			...
			if (target != null) {
				this.adapter.addToObject(target);
				targetActivity = target.getActivity();
			}

			if (targetActivity != null) {
				this.adapter.addToObject(targetActivity);

				EObject targetParent = targetActivity.eContainer();
				if (targetParent != null) {
					this.adapter.addToObject(targetParent);
				}
			}
		}
	}
	...
	protected IFigure createFigure() {
		...
		BPELUIPlugin plugin = BPELUIPlugin.getDefault();
	...
	protected void refreshDecorations() {
		...
		if (markerToDisplay != null) {
			...
			if (hasCondition()) {
				Image image = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_LINK_CONDITIONAL);
				this.decorationImage = null;
