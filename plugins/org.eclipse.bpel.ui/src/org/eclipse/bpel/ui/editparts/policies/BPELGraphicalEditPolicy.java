...
public class BPELGraphicalEditPolicy extends GraphicalNodeEditPolicy {
	...
	private int getLinkMode(CreateConnectionRequest req) {
		if (req.getStartCommand().getClass().equals(SetCompensateCommand.class))
			return BPELGraphicalEditPolicy.COMPENSATELINK;
		return BPELGraphicalEditPolicy.FLOWLINK;
	}
	...
	private PolylineConnection createConnection(ConnectionAnchor sourceAnchor, ConnectionAnchor targetAnchor, boolean sourceArrow, boolean targetArrow) {
		...
		connection.setForegroundColor(BPELUIPlugin.getDefault().getColorRegistry().get(IBPELUIConstants.COLOR_BLUE));
		connection.setBackgroundColor(BPELUIPlugin.getDefault().getColorRegistry().get(IBPELUIConstants.COLOR_BLUE));
		connection.setConnectionRouter(new ManhattanConnectionRouter());
		PolygonDecoration arrow = new PolygonDecoration();
		arrow.setTemplate(PolygonDecoration.TRIANGLE_TIP);
		arrow.setScale(5,2.5);
		arrow.setBackgroundColor(BPELUIPlugin.getDefault().getColorRegistry().get(IBPELUIConstants.COLOR_BLUE));
		if (targetArrow) {
	...
	protected Connection createDummyConnection(Request req) {
		if (getLinkMode((CreateConnectionRequest)req) == BPELGraphicalEditPolicy.FLOWLINK)
	...
	protected Command getConnectionCompleteCommand(CreateConnectionRequest request) {
		...
		if (getLinkMode(request) == BPELGraphicalEditPolicy.FLOWLINK) {
		...
		else if (getLinkMode(request) == BPELGraphicalEditPolicy.COMPENSATELINK) {
	...
	protected ConnectionAnchor getTargetConnectionAnchor(CreateConnectionRequest request) {
		...
		// VZ: this code is false (can lead to ClassCastException
		//		else
	...
	public Command getCommand(Request request) {
		if( request instanceof CreateRequest
					&& "changeType".equals(request.getType())) //$NON-NLS-1$
