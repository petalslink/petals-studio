	...
	protected IFigure createFigure() {
		...
		if (image == null) {
			...
			if (getModel() instanceof EventHandler) {
				image = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_EVENT_INDICATOR);
			} else if (getModel() instanceof CompensationHandler) {
				image = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_COMPENSATION_INDICATOR);
			} else if (getModel() instanceof TerminationHandler) {
				image = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_TERMINATION_INDICATOR);
			} else {
				image = BPELUIPlugin.getDefault().getImage(IBPELUIConstants.ICON_FAULT_INDICATOR);
			}		
		}
