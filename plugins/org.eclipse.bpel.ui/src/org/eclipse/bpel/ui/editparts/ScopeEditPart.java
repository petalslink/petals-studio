...
public class ScopeEditPart extends CollapsableEditPart {
			...
			Display.getDefault().asyncExec( new Runnable() {
				public void run() {
					if(ModelHelper.isHorizontalLayout(getModel()))
						setBorder(new MarginBorder(ScopeEditPart.this.contentFigure.getPreferredSize().height+10,0,0,0));
					else
						setBorder(null);
				}
			});
	...
	protected IFigure createFigure() {
		...
		if (this.collapsedImage == null) {
			...
			this.collapsedImage = element.getSmallImage(getActivity());
		}
		...
		this.editPartMarkerDecorator = new BPELEditPartMarkerDecorator((EObject)getModel(),
		...
		this.parentFigure.setLayoutManager(layout);

		this.contentFigure = new CollapsableScopeContainerFigure(getModel(), getImage(), getLabel());
		this.contentFigure.addMouseMotionListener(getMouseMotionListener());
		this.contentFigure.setEditPart(this);
		this.contentFigure.setForegroundColor(BPELUIPlugin.getDefault().getColorRegistry().get(IBPELUIConstants.COLOR_BLACK));
		this.parentFigure.add(this.contentFigure);

		// Configure the border and contents based on collapsed state
		if (isCollapsed()) {
			configureCollapsedFigure(this.contentFigure);
		} else {
		...
		this.auxilaryFigure.setLayoutManager(layout);
	...
	public ConnectionAnchor getConnectionAnchor(int location) {
		...
		case CenteredConnectionAnchor.LEFT:
			return new CenteredConnectionAnchor(this.contentFigure, CenteredConnectionAnchor.LEFT_INNER, 0);
		case CenteredConnectionAnchor.RIGHT:
			return new CenteredConnectionAnchor(this.contentFigure, CenteredConnectionAnchor.RIGHT_INNER, 0);
		default:
		...
		return new BPELDragEditPartsTracker(this) {
			...
			protected boolean handleButtonDown(int button) {
				...
				if (border.isPointInFaultImage(point.x, point.y)) {
					setShowFaultHandler(!ScopeEditPart.this.showFH);
					return true;
				}
				if (border.isPointInCompensationImage(point.x, point.y)) {
					setShowCompensationHandler(!ScopeEditPart.this.showCH);
					return true;
				}
				if (border.isPointInTerminationImage(point.x, point.y)) {
					setShowTerminationHandler(!ScopeEditPart.this.showTH);
					return true;
				}
				if (border.isPointInEventImage(point.x, point.y)) {
					setShowEventHandler(!ScopeEditPart.this.showEH);
					return true;
				}
	...
	protected void configureCollapsedFigure(IFigure figure) {
		...
		figure.add(this.collapsedImageLabel);
		figure.add(this.collapsedNameLabel);
	}
