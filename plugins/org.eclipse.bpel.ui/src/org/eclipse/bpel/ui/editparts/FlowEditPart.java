...
public class FlowEditPart extends CollapsableEditPart {
	...
	private final EContentAdapter flowContentAdapter;

	private final BatchedMultiObjectAdapter flowBatchedAdapter;
	...
	class FlowDecorationLayout extends BPELDecorationLayout {
		...
		protected Point calculateLocation(int locationHint, IFigure container,
			...
			case PositionConstants.BOTTOM | PositionConstants.LEFT: {
				...
				- (FlowEditPart.this.image.getBounds().height / 2);
				return new Point(x, y);
			}
	...
	public void setCollapsed(boolean collapsed) {
		if (collapsed != this.collapsed) {
			if (this.flowHighlightEditPolicy != null) {
	...
	protected IFigure createFigure() {
		...
		this.editPartMarkerDecorator = new BPELEditPartMarkerDecorator(
					(EObject) getModel(), new FlowDecorationLayout());
		this.editPartMarkerDecorator
	...
	public void deactivate() {
		...
		.removeCommandStackListener(this.stackListener);
	}

	private boolean getShowFreeformFlow() {
		return BPELUIPlugin.getDefault().getPreferenceStore().getBoolean(
	...
	public void doImmediateAutoLayout() {
		...
		if (BPELUIPlugin.getDefault().getPreferenceStore().getBoolean(
	...
	private static class EditPartEdge {
		private final EditPartNode source, dest;
	...
	private static class EditPartNode {
		...
		public boolean visit() {
			EditPartEdge e;
			if (this.visited == VISITING) {
				return false;
			}
			if (this.visited == VISITED)
				return true;
			this.visited = VISITING;
			for (Iterator it = this.edges.iterator(); it.hasNext();) {
