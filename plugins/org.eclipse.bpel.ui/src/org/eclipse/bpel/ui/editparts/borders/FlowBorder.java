...
public class FlowBorder extends GradientBorder {
	...
	public void paint(IFigure figure, Graphics graphics, Insets insets) {
		...
		int topDrawerY = this.bounds.y + IBPELUIConstants.ARC_WIDTH;
		int bottomDrawerX = this.bounds.x;
		int bottomDrawerY = this.bounds.y + IBPELUIConstants.ARC_WIDTH + DRAWER_HALF_HEIGHT;
		...
		if (topMarker != null || bottomMarker != null) {
			...
			ColorRegistry registry = BPELUIPlugin.getDefault().getColorRegistry();
		...
		Rectangle roundBounds = this.bounds.getCopy();
	...
	protected Rectangle getGradientRect() {
		...
		if (ep != null) {
			...
			if ( ep instanceof  FlowEditPart && !((FlowEditPart)ep).isCollapsed()) {
				rect = rect.getCopy();
	...
	public boolean isPointInTopDrawer(int x, int y) {
		...
		this.parentFigure.translateToRelative(p);
		Image image = getTopImage();
		org.eclipse.swt.graphics.Rectangle imageSize = image.getBounds();
		int imageX = this.bounds.x + DRAWER_INSET;
		int imageY = this.bounds.y + DRAWER_INSET + IBPELUIConstants.ARC_WIDTH;
	...
	public boolean isPointInBottomDrawer(int x, int y) {
		...
		org.eclipse.swt.graphics.Rectangle imageSize = image.getBounds();
		int imageX = this.bounds.x + DRAWER_INSET;
		int imageY = this.bounds.y + DRAWER_INSET + DRAWER_HALF_HEIGHT + IBPELUIConstants.ARC_WIDTH;
		Rectangle imageBounds = new Rectangle(imageX, imageY, imageSize.width, imageSize.height);
		return imageBounds.contains(p);
	}
}
