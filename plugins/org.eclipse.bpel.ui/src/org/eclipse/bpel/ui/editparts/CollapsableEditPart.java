...
public abstract class CollapsableEditPart extends CompositeActivityEditPart implements ILayoutAware {
	...
	public class CollapsableDecorationLayout extends BPELDecorationLayout {
		private final int borderImageWidth;
		...
		protected Point calculateLocation(int locationHint, IFigure container, Dimension childDimension) {
			...
			case PositionConstants.TOP | PositionConstants.LEFT: {
				...
				if (!isCollapsed()) {
					y += (CollapsableEditPart.this.collapsedLabel.getSize().height / 2) - (IBPELUIConstants.ARC_WIDTH / 2);
				}
				return new Point(x, y);
			}
	...
	protected IFigure createFigure() {
		...
		this.editPartMarkerDecorator = new BPELEditPartMarkerDecorator(
				(EObject)getModel(),
				new CollapsableDecorationLayout(this.image.getBounds().width)
		);
		this.editPartMarkerDecorator.addMarkerMotionListener(getMarkerMotionListener());

		IFigure figure = getNewContentPane(this.editPartMarkerDecorator.getDecorationLayer());
		this.contentFigure = figure;

		figure.setForegroundColor(BPELUIPlugin.getDefault().getColorRegistry().get(IBPELUIConstants.COLOR_BLACK));
		if (isCollapsed()) {
	...
	protected IFigure getNewContentPane(Layer layer) {
		CollapsableContainerFigure fig =  new CollapsableContainerFigure(getModel(), this.image, getLabel());
	...
	protected void addCollapsedContents(IFigure figure) {
		...
		if (!isGenericContainerBorder()) {
			...
			figure.add(this.collapsedLabel);
		}
	}
	@Override
