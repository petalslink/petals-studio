...
public class BPELOrderedLayoutEditPolicy extends FlowLayoutEditPolicy {
	...
	protected Color arrowColor = BPELUIPlugin.getDefault().getColorRegistry().get(IBPELUIConstants.COLOR_IMPLICIT_LINK);
	...
	/*
	 * Needs to be overwritten to prevent a ClassCastException
	 * (non-Jsdoc)
	 * @see org.eclipse.gef.editpolicies.OrderedLayoutEditPolicy
	 * #isLayoutHorizontal()
	 */
	@Override
	protected boolean isLayoutHorizontal() {
		...
		boolean result;
		LayoutManager lm = getLayoutContainer().getLayoutManager();
		if( lm instanceof OrderedLayout )
			result = ((OrderedLayout) lm).isHorizontal();
		else if( lm instanceof AlignedFlowLayout )
			result = ((AlignedFlowLayout) lm).isHorizontal();
		else
			result = false;
