...
public class Templates {
	...
	public void initializeFrom ( String templateLocation ) {
		initializeFrom(BPELUIPlugin.getDefault().getBundle(), templateLocation);
	}
	...
	public void initializeFrom  (Bundle bundle, String templateLocation ) {
		...
		while (list.hasMoreElements()) {
			...
			this.mNamespaceNames.addAll( findProperties (props,"namespace.{0}") );
	...
	List<String> findProperties ( Properties props, String pattern ) {
		...
		for(int i=0; hole >= 0; i++) {
			...
			String key = MessageFormat.format(pattern, new Object[] { Integer.valueOf( i )});
	...
	public static class TemplateResource {
		...
		public String process ( Map<String,Object> args ) {
			...
			return process ( this.mContent, args );
		}
		...
		Object lookup ( String key, Map<String,Object> args) {
			...
			if (key.startsWith(":include:")) {
				key = key.substring(9);
				r = this.mTemplate.lookupResource(key);
				if (r != null) {
					value = r.mContent;
				}
		...
		public String getName (Map<String,Object> args) {
			...
			return process (this.mNameTemplate, args);
		}
		...
		{
			return this.mName.endsWith( BPEL_FILE_EXTENSION );
		}
	}
