...
public class PartnerLinkImplSection extends BPELPropertySection {
	...
	protected void addAllAdapters() {
		...
		if (this.fAdapters.length > 0) {
			...
			if (partnerLink.getPartnerLinkType() != null) {
				this.fAdapters[0].addToObject(partnerLink.getPartnerLinkType());
			}
			Role[] roles = {partnerLink.getMyRole(), partnerLink.getPartnerRole()};
			for( Role role : roles ) {
				if (role != null) {
					this.fAdapters[0].addToObject(role);
				}
			}
		}
	}
	...
	protected Composite createPartnerLinkTypeWidgets (Composite top, final Composite parent) {
		...
		this.partnerLinkTypeBrowseButton = this.fWidgetFactory.createButton(composite, Messages.PartnerLinkImplSection_Browse_1, SWT.PUSH);

		DecoratedLabel label = new DecoratedLabel ( composite, SWT.LEFT );
		label.setText( Messages.PartnerLinkImplSection_0 );
		this.fWidgetFactory.adapt(label);

		this.fPartnerLinkTypeLabel = new StatusLabel2 ( label );

		this.fPartnerLinkTypeHref = this.fWidgetFactory.createHyperlink(composite, "", SWT.NONE); //$NON-NLS-1$
		this.fPartnerLinkTypeHref.addHyperlinkListener(new HyperlinkAdapter() {
		...
		this.partnerLinkTypeBrowseButton.setLayoutData(data);

		data = new FlatFormData();
		data.left = new FlatFormAttachment(0,IDetailsAreaConstants.HSPACE);
		data.top = new FlatFormAttachment(0, IDetailsAreaConstants.VSPACE );
		this.fPartnerLinkTypeLabel.setLayoutData(data);

		data = new FlatFormData();
		data.right = new FlatFormAttachment( 60, 0 );
		data.left = new FlatFormAttachment ( this.fPartnerLinkTypeLabel.getLabel() , 20);
		data.bottom = new FlatFormAttachment ( this.fPartnerLinkTypeLabel.getLabel(), -1 , SWT.BOTTOM);
		this.fPartnerLinkTypeHref.setLayoutData(data);
	...
	protected void browseForPartnerLink ( ) {
		...
		Object obj = BrowseUtil.browseForPartnerLinkType(
					getInput(),
					this.partnerLinkTypeBrowseButton.getShell() );
	...
	protected Composite createMyRolePartnerRoleWidgets ( Composite top, Composite parent) {
		...
		this.fMyRole1 = createButton(myGroup, null, BID_MY_ROLE_1,    false, SWT.RADIO );
		this.fMyRole2 = createButton(myGroup, null, BID_MY_ROLE_2,    false, SWT.RADIO);
		this.fMyRoleNone = createButton(myGroup, Messages.PartnerLinkImplSection_10, BID_MY_ROLE_NONE, true,  SWT.RADIO);
		...
		this.fPartnerRole1 = createButton(partnerGroup, null, BID_PARTNER_ROLE_1,    false, SWT.RADIO);
		this.fPartnerRole2 = createButton(partnerGroup, null, BID_PARTNER_ROLE_2,    false, SWT.RADIO);
		this.fPartnerRoleNone = createButton(partnerGroup, Messages.PartnerLinkImplSection_5, BID_PARTNER_ROLE_NONE, true, SWT.RADIO);

		// The initialize partner role button
		this.fInitializePartnerRole = createButton(partnerGroup, Messages.PartnerLinkImplSection_11, BID_INITIALIZE_PARTNER_ROLE, false, SWT.CHECK);
	...
	protected Composite createPartnerLinkStructureWidgets (Composite top, Composite parent) {
		...
		Tree tree  = this.fWidgetFactory.createTree(composite, SWT.NONE);
		...
		// Tree viewer for variable structure ...
		tree = this.fWidgetFactory.createTree(composite, SWT.NONE);
	...
	protected void createClient(Composite parent) {
		...
		PlatformUI.getWorkbench().getHelpSystem().setHelp(
					this.parentComposite, IHelpContextIds.PROPERTY_PAGE_PARTNER_IMPLEMENTATION);
	}
	...
	protected void updateMyRolePartnerRoleWidgets ( PartnerLink pl ) {
		...
		if (plt != null) {
			this.fRoles = plt.getRole().toArray( NO_ROLES );
		}
		...
		if (this.fRoles.length >= 1) {
			...
			updateRadio (this.fMyRole1, role1);
			updateRadio (this.fPartnerRole1,role1);
		...
		if (this.fRoles.length >= 2) {
			...
			updateRadio (this.fMyRole2, role2);
			updateRadio (this.fPartnerRole2,role2);
			...
			this.fInitializePartnerRole.setSelection(ipr != null && ipr );
	...
	protected void updatePartnerLinkTypeWidgets() {
		...
		this.fMyOperationsTreeViewer.setInput( ModelHelper.getPartnerPortType(pl, ModelHelper.INCOMING) );
		this.fPartnerOperationsTreeViewer.setInput( ModelHelper.getPartnerPortType(pl, ModelHelper.OUTGOING ) );
	...
	protected void buttonPressed(int id, boolean selection, boolean event ) {
		...
		case BID_MY_ROLE_1 :
			...
			if (myRole == null || myRole.equals( this.fRoles[0]) == false ) {
				cmd = new SetCommand(pl,this.fRoles[0],BPELPackage.eINSTANCE.getPartnerLink_MyRole() );
			}
			break;

		case BID_MY_ROLE_2 :
			if (myRole == null || myRole.equals( this.fRoles[1]) == false ) {
				cmd = new SetCommand(pl,this.fRoles[1],BPELPackage.eINSTANCE.getPartnerLink_MyRole() );
			}
			break;
		...
		case BID_PARTNER_ROLE_1 :
			if (partnerRole == null || partnerRole.equals( this.fRoles[0]) == false) {
				cmd = new SetCommand(pl,this.fRoles[0],BPELPackage.eINSTANCE.getPartnerLink_PartnerRole() );
			}
			break;

		case BID_PARTNER_ROLE_2 :
			if (partnerRole == null || partnerRole.equals( this.fRoles[1]) == false) {
				cmd = new SetCommand(pl,this.fRoles[1],BPELPackage.eINSTANCE.getPartnerLink_PartnerRole() );
			}
			break;
		...
		case BID_INITIALIZE_PARTNER_ROLE :
			cmd = new SetCommand(pl,this.fInitializePartnerRole.getSelection(),BPELPackage.eINSTANCE.getPartnerLink_InitializePartnerRole() );
			break;
		}
	...
	protected void updateMarkers () {
		...
		for(IMarker m : getMarkers(getInput())) {
			this.fPartnerLinkTypeLabel.addStatus( BPELUtil.adapt(m, IStatus.class));
		}
	}
