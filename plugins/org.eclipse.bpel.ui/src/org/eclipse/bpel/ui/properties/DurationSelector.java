...
public class DurationSelector extends Composite {
	...
	private void createControls(Composite parent){
		...
		for (int i = 0; i<6; i++) {
			...
			this.composite[i].setLayoutData(new GridData(GridData.FILL_HORIZONTAL));

			FlatFormLayout formLayout = new FlatFormLayout();
			formLayout.marginWidth = formLayout.marginHeight = 0;
			this.composite[i].setLayout(formLayout);

			// Accessibility: create each Label right before the corresponding Text.
			this.label[i] = this.wf.createLabel(this.composite[i], labelStrings[i], SWT.CENTER);
			this.text[i] = this.wf.createText(this.composite[i], "0"); //$NON-NLS-1$
			...
			this.label[i].setLayoutData(data);

			data = new FlatFormData();
			data.left = new FlatFormAttachment(0,0);
			data.right = new FlatFormAttachment(100,0);
			data.top = new FlatFormAttachment(this.label[i], IDetailsAreaConstants.VSPACE);
			this.text[i].setLayoutData(data);
		}
		...
		FocusListener focusListener = new FocusListener() {
			...
			public void focusLost(FocusEvent e) {
				...
				if (ct.equals(ov) == false) {
					DurationSelector.this.lastWidgetChanged = w.getData( ID );
					selectionChanged();
				}
			}
		};
	...
	public int[] getValues() {
		...
		for (int i = 0; i<6; i++) {
			result[i] = numberValue(this.text[i].getText());
		}
		return result;
	}
