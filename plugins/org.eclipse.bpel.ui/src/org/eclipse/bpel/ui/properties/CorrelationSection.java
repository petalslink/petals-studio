...
public class CorrelationSection extends BPELPropertySection {
	...
	public class DirectionColumn extends ColumnTableProvider.Column implements ILabelProvider, ICellModifier {
		...
		public String getText(Object element) {
			if (CorrelationSection.this.allowOutgoing && CorrelationSection.this.allowIncoming) {
				return this.twoWayStrings[patternIndex(((Correlation)element).getPattern())];
			}
	...
	public class InitiationColumn extends ColumnTableProvider.Column implements ILabelProvider, ICellModifier {
		...
		public String getText(Object element) {
			return this.strings[initiationIndex(((Correlation)element).getInitiate())];
		}
		public boolean canModify(Object element, String property) {
			return true;
		}
	...
	public class NameColumn extends ColumnTableProvider.Column implements ILabelProvider, ICellModifier {
		...
		public CellEditor createCellEditor(Composite parent) {
			...
			this.cellEditor.setStyle(SWT.READ_ONLY);
					...
					new MultiObjectAdapter() {
						...
						public void notify(Notification n) {
							if (n.getNotifier() instanceof Correlation) {
								...
								if (c != null) {
									for( Object element : c.getChildren() ) {
										Correlation corr = (Correlation)element;
										CorrelationSection.this.fAdapters[1].addToObject(corr);
	...
	protected void addAllAdapters() {
		...
		Correlations c = ModelHelper.getCorrelations(getInput());
		if (c != null) {
			this.fAdapters[0].addToObject(c);
			for( Object element : c.getChildren() ) {
				Correlation corr = (Correlation)element;
				this.fAdapters[1].addToObject(corr);
				if (corr.getSet() != null)  this.fAdapters[1].addToObject(corr.getSet());
			}
		}
	}
	...
	protected Correlation newDefaultCorrelation() {
		...
		for( Object element : getProcess().getCorrelationSets().getChildren() ) {
			CorrelationSet set = (CorrelationSet)element;
	...
	protected void createCorrelationSetWidgets(Composite composite)  {
		...
		this.addButton = this.fWidgetFactory.createButton(composite, Messages.CorrelationDetails_Add_18, SWT.PUSH);
		this.removeButton = this.fWidgetFactory.createButton(composite, Messages.CorrelationDetails_Remove_19, SWT.PUSH);
		Label correlationSetLabel = this.fWidgetFactory.createLabel(composite, Messages.CorrelationDetails_Correlation_Sets__20);
		this.correlationTable = this.fWidgetFactory.createTable(composite, SWT.FULL_SELECTION | SWT.V_SCROLL);
		...
		data = new FlatFormData();
		data.width = BPELUtil.calculateButtonWidth(this.addButton, STANDARD_BUTTON_WIDTH);
		data.right = new FlatFormAttachment(this.removeButton, -IDetailsAreaConstants.HSPACE);
		data.top = new FlatFormAttachment(0, 0);
		this.addButton.setLayoutData(data);

		data = new FlatFormData();
		data.width = BPELUtil.calculateButtonWidth(this.removeButton, STANDARD_BUTTON_WIDTH);
		...
		this.addButton.addSelectionListener(new SelectionListener() {
			public void widgetSelected(SelectionEvent e) {
				...
				getCommandFramework().execute(wrapInShowContextCommand(cmd));
				CorrelationSection.this.correlationViewer.setSelection(new StructuredSelection(correlation));
		...
		Listener removeListener = new Listener() {
			public void handleEvent(Event e) {
				...
				int items = CorrelationSection.this.correlationTable.getItemCount();
				if (items > 0) {
		...
		this.correlationTable.addListener(SWT.KeyUp, removeListener);

		this.removeButton.addListener(SWT.Selection, removeListener);
		this.removeButton.addListener(SWT.DefaultSelection, removeListener);


		data = new FlatFormData();
		data.left = new FlatFormAttachment(0, 0);
		data.right = new FlatFormAttachment(100, 0);
		data.top = new FlatFormAttachment(this.addButton, IDetailsAreaConstants.VSPACE);
		data.bottom = new FlatFormAttachment(100, 0);
		this.correlationTable.setLayoutData(data);
	...
	protected void updateCorrelationWidgets(Correlation corr) {
		...
		this.allowOutgoing = ModelHelper.isMessageActivity(input, ModelHelper.OUTGOING);
		this.allowIncoming = ModelHelper.isMessageActivity(input, ModelHelper.INCOMING);
		this.correlationViewer.setInput(getInput());
		if (corr != null) {
			this.correlationViewer.refresh(corr, true);
		} else {
	...
	public void restoreUserContext(Object userContext) {
		...
		if (userContext != null) {
			this.correlationViewer.setSelection(new StructuredSelection(userContext));
		}
	}
