...
public abstract class AssignCategoryBase extends BPELPropertySection implements IAssignCategory {
		...
		return new BatchedMultiObjectAdapter[] {
					...
					new BatchedMultiObjectAdapter() {
						...
						public void notify (Notification n) {
							this.bUpdate = isToOrFromAffected(n) ;
						}
	...
	public void createControls (Composite parent, TabbedPropertySheetPage aTabbedPropertySheetPage) {
		...
		if( aTabbedPropertySheetPage instanceof BPELTabbedPropertySheetPage ) {
	...
	protected void basicSetInput (EObject newInput) {
		this.fCopyRuleSide = BPELUtil.adapt(newInput, IVirtualCopyRuleSide.class);
	...
	public Command newStoreModelCommand()  {
		return new UpdateModelCommand(this.fCopyRuleSide.getCopyRuleSide(),"Modify ..!") {
			...
			public void doExecute() {
				store ( AssignCategoryBase.this.fCopyRuleSide );
			}
		};
	}
	...
	protected void createClient2(Composite parent) {
		if (isDefaultCompositeOpaque()) {
			this.fComposite = this.fWidgetFactory.createPlainComposite(parent, SWT.NONE);
		} else {
			this.fComposite = this.fWidgetFactory.createComposite(parent);
		}
		FlatFormLayout layout = new FlatFormLayout();
