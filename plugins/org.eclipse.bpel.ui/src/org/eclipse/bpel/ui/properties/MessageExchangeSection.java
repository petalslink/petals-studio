...
public class MessageExchangeSection extends BPELPropertySection {
	...
	protected void createMessExWidgets(Composite composite) {
		...
		this.messExName 	= this.fWidgetFactory.createText(composite, EMPTY_STRING, SWT.NONE);
		this.messExBrowseButton = this.fWidgetFactory.createButton(composite,EMPTY_STRING,SWT.ARROW | SWT.DOWN | SWT.RIGHT );
		...
		this.messExBrowseButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				contentAssist.openProposals();
			}
		});

		this.messExName.addListener(SWT.KeyDown, new Listener () {
			public void handleEvent(Event event) {
				if (event.keyCode == SWT.CR) {
					findAndSetOrCreateMessageExchange( MessageExchangeSection.this.messExName.getText() );
				}
			}
		});
		...
		data.top 			= new FlatFormAttachment(this.messExName,+2,SWT.TOP);
		data.bottom 		= new FlatFormAttachment(this.messExName,-2,SWT.BOTTOM);
		this.messExBrowseButton.setLayoutData(data);

		data = new FlatFormData();
		data.left = new FlatFormAttachment(0, BPELUtil.calculateLabelWidth(this.messExLabel, STANDARD_LABEL_WIDTH_SM));
		data.right = new FlatFormAttachment(this.messExBrowseButton, 0);
		this.messExName.setLayoutData(data);

		data = new FlatFormData();
		data.left = new FlatFormAttachment(0, 0);
		data.right = new FlatFormAttachment(this.messExName, -IDetailsAreaConstants.HSPACE);
		data.top = new FlatFormAttachment(this.messExName, 0, SWT.CENTER);
		this.messExLabel.setLayoutData(data);
	}
	...
	private void updateMessExWidgets() {
		...
		if (messEx == null) {
			this.messExName.setText(EMPTY_STRING);
		} else {
