...
public class InvokeVariableSection extends BPELPropertySection {
	...
	private final VariableFilter fInputVariableFilter = new VariableFilter();
	private final VariableFilter fOutputVariableFilter = new VariableFilter();
			...
			if (fromParts != null) {
				...
				if (fromParts.getChildren() != null) {
					...
					for (FromPart fromPart : fromParts.getChildren()) {
						this.fAdapters[0].addToObject(fromPart);
					}
				}
			}
		} catch (IllegalArgumentException e) {
			...
			if (toParts != null) {
				...
				if (toParts.getChildren() != null) {
					...
					for (ToPart toPart : toParts.getChildren()) {
						this.fAdapters[0].addToObject(toPart);
					}
				}
			}
		} catch (IllegalArgumentException e) {
	...
	private Composite createInputVariableWidgets(Composite top, Composite parent) {
		...
		final Composite composite = this.inputVariableComposite = createFlatFormComposite(parent);
		...
		this.inputVariableLabel = this.fWidgetFactory.createLabel(composite,
					Messages.InvokeImplSection_7);
		this.inputVariableText = this.fWidgetFactory.createText(composite, EMPTY_STRING);
		this.inputVariableButton = this.fWidgetFactory.createButton(composite,
		...
		final FieldAssistAdapter contentAssist = new FieldAssistAdapter(
					this.inputVariableText, this.fTextContentAdapter, proposalProvider, null,
					null, true);
		//
		...
		.addContentProposalListener(new IContentProposalListener() {
			...
			public void proposalAccepted(IContentProposal chosenProposal) {
				if (chosenProposal.getContent() != null) {
					if( chosenProposal instanceof Adapter
								&& ((Adapter) chosenProposal).getTarget() instanceof Variable ) {

						Variable variable = (Variable) ((Adapter) chosenProposal).getTarget();
		...
		this.inputVariableButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				contentAssist.openProposals();
			}
		});

		this.inputVariableText.addListener(SWT.KeyDown, new Listener() {
			public void handleEvent(Event event) {
				if (event.keyCode == SWT.CR) {
					findAndSetOrCreateVariable(InvokeVariableSection.this.inputVariableText.getText(),
								isInvoke() ? ModelHelper.OUTGOING
											: ModelHelper.INCOMING);
				}
			}
		...
		data.top = new FlatFormAttachment(this.inputVariableText, +2, SWT.TOP);
		data.bottom = new FlatFormAttachment(this.inputVariableText, -2, SWT.BOTTOM);
		this.inputVariableButton.setLayoutData(data);

		data = new FlatFormData();
		data.left = new FlatFormAttachment(0, BPELUtil.calculateLabelWidth(
					null, STANDARD_LABEL_WIDTH_SM));
		data.right = new FlatFormAttachment(this.inputVariableButton, 0);
		this.inputVariableText.setLayoutData(data);

		data = new FlatFormData();
		data.left = new FlatFormAttachment(0, 0);
		data.right = new FlatFormAttachment(this.inputVariableText,
					-IDetailsAreaConstants.HSPACE);
		data.top = new FlatFormAttachment(this.inputVariableText, 0, SWT.CENTER);
		this.inputVariableLabel.setLayoutData(data);
	...
	private Composite createOutputVariableWidgets( Composite top, Composite parent ) {
		...
		this.outputVariableText = this.fWidgetFactory.createText(composite, EMPTY_STRING);
		this.outputVariableButton = this.fWidgetFactory.createButton(composite,
					EMPTY_STRING, SWT.ARROW | SWT.DOWN | SWT.CENTER);
		...
		final FieldAssistAdapter contentAssist = new FieldAssistAdapter(
					this.outputVariableText, this.fTextContentAdapter, proposalProvider,
					null, null, true);
		//
		contentAssist.setLabelProvider(new ModelLabelProvider());
		contentAssist.setPopupSize(new Point(300, 100));
		...
		this.outputVariableButton.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event event) {
				contentAssist.openProposals();
			}
		});
		this.outputVariableText.addListener(SWT.KeyDown, new Listener() {
			public void handleEvent(Event event) {
				if (event.keyCode == SWT.CR) {
					findAndSetOrCreateVariable(InvokeVariableSection.this.outputVariableText.getText(),
								ModelHelper.INCOMING);
				}
			}
		});
		...
		data = new FlatFormData();
		data.right = new FlatFormAttachment(100, 0);
		data.top = new FlatFormAttachment(this.outputVariableText, +2, SWT.TOP);
		data.bottom = new FlatFormAttachment(this.outputVariableText, -2, SWT.BOTTOM);
		this.outputVariableButton.setLayoutData(data);

		data = new FlatFormData();
		data.left = new FlatFormAttachment(0, BPELUtil.calculateLabelWidth(
					null, STANDARD_LABEL_WIDTH_SM));
		data.right = new FlatFormAttachment(this.outputVariableButton, 0);
		this.outputVariableText.setLayoutData(data);

		data = new FlatFormData();
		data.left = new FlatFormAttachment(0, 0);
		data.right = new FlatFormAttachment(this.outputVariableText,
					-IDetailsAreaConstants.HSPACE);
		data.top = new FlatFormAttachment(this.outputVariableText, 0, SWT.CENTER);
		this.outputVariableLabel.setLayoutData(data);
	...
	private void createPartMappingRows(ILabelProvider labelProvider,
		...
		for (final Part part : messageParts) {
			...
			// create text
			final Text text = this.fWidgetFactory.createText(refGroup, EMPTY_STRING);
			Variable var = null;
			...
			if (model instanceof OnEvent == false) {
				...
				// create button
				button = this.fWidgetFactory.createButton(refGroup, EMPTY_STRING,
			...
			final FieldAssistAdapter contentAssist = new FieldAssistAdapter(
						text, this.fTextContentAdapter, proposalProvider, null, null,
						true);
			...
			.addContentProposalListener(new IContentProposalListener() {
				...
				public void proposalAccepted(IContentProposal proposal) {
					if (proposal.getContent() != null) {
						if( proposal instanceof Adapter
									&& ((Adapter) proposal).getTarget() instanceof Variable ) {

							Variable variable = (Variable) ((Adapter) proposal).getTarget();
							Command cmd = null;
							if (isFromParts)
	...
	private void createVariable(EObject ref, String name, int direction) {
		...
		NameDialog nameDialog = new NameDialog(this.inputVariableComposite
	...
	private void createWidgets(Composite composite) {
		...
		// create the widgets
		this.usePartMappingCheckbox = this.fWidgetFactory.createButton(composite,
		...
		Composite ref = createInputVariableWidgets(null,
					this.nonPartMappingComposite);
		createOutputVariableWidgets(ref, this.nonPartMappingComposite);
	...
	private void updateInputVariableWidgets() {
		...
		if (inputVar != null) {
			...
			this.inputVariableText.setText(EMPTY_STRING);
		}
	...
	private void updateOutputVariableWidgets() {
		...
		if (outputVar != null) {
			...
			this.outputVariableText.setText(EMPTY_STRING);
		}
	...
	private void updatePartMappingWidgets() {
		...
		if (operation != null && !operation.eIsProxy()) {
			...
			if (inputParts.size() > 0) {
				...
				refGroup = this.fWidgetFactory.createGroup(
