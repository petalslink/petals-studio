...
public abstract class AbstractHandlerLinker {
	 ...
	 */
	private final List<PolylineConnection> handlerConnectionsList = new ArrayList<PolylineConnection>();

	/**
	 * The editPart to which this handler belongs to
	 */
	private final BPELEditPart editPart;

	/**
	 * Reference to the GEF connectionLayer
	 */
	private final IFigure connLayer;

	public AbstractHandlerLinker(BPELEditPart editPart) {
		this.editPart = editPart;
		this.connLayer = ((LayerManager) editPart.getViewer().getEditPartRegistry()
				.get(LayerManager.ID))
	...
	public void refreshHandlerLinks() {
		...
		if (isShowFH()) {
			...
			conn.setForegroundColor(BPELUIPlugin.getDefault().getColorRegistry()
	...
	protected boolean isHorizontalLayout() {
		return ModelHelper.getBPELEditor(this.editPart.getModel())
		.isHorizontalLayout();
	}
