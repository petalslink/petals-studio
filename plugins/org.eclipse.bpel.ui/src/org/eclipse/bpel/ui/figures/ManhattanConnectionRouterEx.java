...
public class ManhattanConnectionRouterEx extends AbstractRouter {
	...
	private final Map rowsUsed = new HashMap();
	private final Map colsUsed = new HashMap();
	//private Hashtable offsets = new Hashtable(7);

	private final Map reservedInfo = new HashMap();
	...
	protected void processPositions(Ray start, Ray end, List positions,
		...
		if (horizontal == (Math.abs( positions.size()) % 2 == 1))
			pos[++i] = end.x;
		else
			pos[++i] = end.y;
	...
	public void route(Connection conn) {
		...
		if(this.prefEndDirection != null && this.prefStartDirection != null){
			startNormal = this.prefStartDirection;
			endNormal = this.prefEndDirection;
		}else{
