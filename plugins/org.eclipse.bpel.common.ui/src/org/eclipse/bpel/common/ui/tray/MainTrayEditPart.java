...
public class MainTrayEditPart extends TrayContainerEditPart {
	...
	protected static class MainTrayTitleFigure
	...
	implements HandleBounds {
		...
		public MainTrayTitleFigure() {
			...
			setBorder( new MarginBorder( new Insets( 6 )));
		}
	...
	protected class MainTraySelectionEditPolicy extends TraySelectionEditPolicy {
		...
		protected Handle createHandle(GraphicalEditPart owner) {
			return new TraySelectionHandle(owner, MainTrayEditPart.this.mainTrayTitleFigure);
		}
	}
	...
	public class MainTrayContainerFigure extends TrayContainerFigure {
		...
		protected class MainTrayContainerFigureLayout extends AbstractLayout {
			...
			protected Dimension calculatePreferredSize(IFigure container, int wHint, int hHint) {
				if (MainTrayContainerFigure.this.titleFigure == null
							|| MainTrayContainerFigure.this.childrenScrollPane == null)
					return new Dimension(wHint, hHint);

				Dimension titleSize = MainTrayContainerFigure.this.titleFigure.getPreferredSize(wHint, hHint);
				Dimension scrollPaneSize = MainTrayContainerFigure.this.childrenScrollPane.getPreferredSize(wHint, hHint);
			...
			public void layout(IFigure container) {
				...
				Dimension size = MainTrayContainerFigure.this.titleFigure.getPreferredSize();
				MainTrayContainerFigure.this.titleFigure.setBounds(new Rectangle(area.x, area.y, area.width, size.height));
				area.y += size.height;
				int height = area.height - size.height;
				MainTrayContainerFigure.this.childrenScrollPane.setBounds(new Rectangle(area.x, area.y, area.width, height));
			}
		...
		public void setChildrenFigure(IFigure target) {
			...
			this.childrenScrollPane.setContents(target);
		}
	...
	protected void refreshVisuals() {
		...
		// VZ
		this.nameLabel.setText( " Dashboard :: " + getLabelProvider().getText( getModel()));
		// VZ
