...
public abstract class CompositeEditor extends EditorPart {
	 ...
	 */
	static class EmbeddedEditorsCollection {
		private final List<IEditorPart> editors;
		private IEditorPart[] editorsArrayCache;
		private int activeEditorIndex;

		EmbeddedEditorsCollection() {
			this.editors = new ArrayList<IEditorPart>(10);
			this.editorsArrayCache = null;
			this.activeEditorIndex = -1; // -1 means there are no active editors
		}
	...
	public CompositeEditor() {
		this.embeddedEditors = new EmbeddedEditorsCollection();
	}
	...
	public IEditorPart connectEditor(String editorId, IEditorInput input, Composite editorComposite) throws CoreException {
		final IEditorPart editor = this.editorManager.createEditor(editorId, input);
	...
	public void doSave(IProgressMonitor monitor) {
		...
		try {
			IEditorPart[] editors = getEmbeddedEditorsSaveOrder(this.embeddedEditors.getEmbeddedEditors());
			...
			this.partListener = new IPartListener() {
				public void partActivated(IWorkbenchPart part) {
					if (part == CompositeEditor.this) {
						IEditorPart lastActiveEditor = CompositeEditor.this.embeddedEditors.getActiveEditor();
