...
public class EditPartMarkerDecorator {
	...
	protected void refreshMarkers() {
		...
		for (List<IMarker> markerList : sortByType ( getMarkers())) {
			...
			final Object constraint = getConstraint(list[0]);
			final IFigure markerFigure = createFigureForMarkers(list);
			if (markerFigure != null) {

				// VZ: run the addition in a separate thread
				try {
					Display.getDefault().asyncExec( new Runnable() {
						public void run() {
							try {
								EditPartMarkerDecorator.this.decorationLayer.add( markerFigure, constraint );
							} catch( Exception e ) {
								e.printStackTrace();
							}
						}
	...
	public void refresh(){
		...
		if(this.decorationLayer != null) {
			...
			Display.getDefault().asyncExec( new Runnable() {
				public void run() {
					EditPartMarkerDecorator.this.decorationLayer.removeAll();
				}
			});
			// VZ
		}
	...
	public IFigure createFigure(IFigure figure) {
		...
		if (this.figureLayout == null) {
			if (this.fillParent) {
				...
				this.figureLayout = new AlignedFlowLayout() {
