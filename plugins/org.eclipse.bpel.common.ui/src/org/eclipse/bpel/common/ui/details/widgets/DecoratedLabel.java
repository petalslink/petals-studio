...
public class DecoratedLabel extends Canvas {
	...
	private static final int hIndent = 0;
	private static final int vIndent = INDENT;
	/** the current text */
	private String text;
	/** the current icon */
	private Image image;
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		checkWidget();
		Point e = getTotalSize(this.image, this.text);
		if (wHint == SWT.DEFAULT){
			e.x += 2*DecoratedLabel.hIndent;
		} else {
			e.x = wHint;
		}
		if (hHint == SWT.DEFAULT) {
			e.y += 2*DecoratedLabel.vIndent;
		} else {
			e.y = hHint;
		}
		return e;
	...
	void onPaint(PaintEvent event) {
		...
		int availableWidth = Math.max(0, rect.width - 2*DecoratedLabel.hIndent);
		...
		int x = rect.x + DecoratedLabel.hIndent;
		if (this.align == SWT.CENTER) {
			x = (rect.width - extent.x)/2;
		}
		if (this.align == SWT.RIGHT) {
			x = rect.width - DecoratedLabel.hIndent - extent.x;
		}
				...
				if (this.gradientColors.length == 1) {
					...
					for (int i = 0; i < this.gradientPercents.length; ++i) {
						...
						if (this.gradientVertical) {
							final int gradientHeight = (this.gradientPercents[i] * rect.height / 100) - pos;
							gc.fillGradientRectangle(0, pos, rect.width, gradientHeight, true);
							pos += gradientHeight;
						} else {
							final int gradientWidth = (this.gradientPercents[i] * rect.width / 100) - pos;
		...
		if (lines != null) {
			...
			int lineY = Math.max(DecoratedLabel.vIndent, rect.y + (rect.height - textHeight) / 2);
			...
			for( String line : lines ) {
				...
				if (lines.length > 1) {
					...
					if (this.align == SWT.RIGHT) {
						int lineWidth = gc.textExtent(line, DRAW_FLAGS).x;
						lineX = Math.max(x, rect.x + rect.width - DecoratedLabel.hIndent - lineWidth);
					}
				}
	...
	public void setBackground(Color[] colors, int[] percents, boolean vertical) {
		...
		if (this.backgroundImage == null) {
			if ((this.gradientColors != null) && (colors != null) &&
						(this.gradientColors.length == colors.length)) {
				boolean same = false;
				for (int i = 0; i < this.gradientColors.length; i++) {
					same = (this.gradientColors[i] == colors[i]) ||
					((this.gradientColors[i] == null) && (colors[i] == background)) ||
					((this.gradientColors[i] == background) && (colors[i] == null));
					if (!same) break;
				}
				if (same) {
					for (int i = 0; i < this.gradientPercents.length; i++) {
