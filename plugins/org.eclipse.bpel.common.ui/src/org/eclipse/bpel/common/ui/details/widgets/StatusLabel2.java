...
public class StatusLabel2 {
	...
	protected static class StatusMessage {
		...
		protected void setStatus (IStatus status) {
			if (status == null) {
				setSeverity( StatusLabel2.STATUS_MESSAGE_ID, IStatus.OK, null);
			} else {
				setSeverity( StatusLabel2.STATUS_MESSAGE_ID, status.getSeverity(), status.getMessage());
			}
		}
	...
	public void setSeverity(int severity, String message) {
		setSeverity(StatusLabel2.STATUS_MESSAGE_ID, severity, message);
	}
	...
	public void addSeverity(int severity, String message) {
		addSeverity(StatusLabel2.STATUS_MESSAGE_ID, severity, message);
	}
	...
	protected String getToolTipText() {
		...
		appendToolTipTextList(toolTipTextList, IStatus.ERROR, StatusLabel2.MAX_MESSAGES);
		appendToolTipTextList(toolTipTextList, IStatus.WARNING, StatusLabel2.MAX_MESSAGES);
		appendToolTipTextList(toolTipTextList, IStatus.INFO, StatusLabel2.MAX_MESSAGES);
		appendToolTipTextList(toolTipTextList, IStatus.OK, StatusLabel2.MAX_MESSAGES);

		for (String t : toolTipTextList) {
			toolTipText.append(t).append(StatusLabel2.NEW_LINE);
		}

		if (this.statusMessageList.size() > toolTipTextList.size()) {
			if (toolTipText.length() > 0) {
				toolTipText.append(StatusLabel2.ETCETERA);
			}
		}
