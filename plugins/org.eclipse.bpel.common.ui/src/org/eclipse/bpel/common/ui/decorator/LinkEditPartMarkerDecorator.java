...
public class LinkEditPartMarkerDecorator extends EditPartMarkerDecorator {
	...
	private final List<IFigure> fMarkerFigures = new ArrayList<IFigure>();
	...
	protected Object getConstraint(IMarker marker) {
		try {
			if (marker.isSubtypeOf(IModelMarkerConstants.DECORATION_GRAPHICAL_MARKER_ID)) {
				...
				if (anchorString != null) {
					...
					if (anchorString.equals(IMarkerConstants.MARKER_ANCHORPOINT_CENTRE)) return new ConnectionLocator(this.fConnection, ConnectionLocator.MIDDLE);
				}
			}
		...
		// default
		return new ConnectionLocator(this.fConnection, ConnectionLocator.MIDDLE);
	}
	...
	protected void refreshMarkers() {
		...
		try {
			...
			if(getConnection() != null) {
				...
				for( Object element : constraintToMarkerMap.entrySet() ) {
