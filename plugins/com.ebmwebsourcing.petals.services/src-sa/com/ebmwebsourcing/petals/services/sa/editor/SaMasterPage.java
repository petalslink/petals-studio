...
implements IServiceMasterPage {
	...
	private final AbstractServicesFormPage saFormPage;
		...
		this.viewer.setContentProvider( new ITreeContentProvider() {
			...
			public Object[] getElements( Object inputElement ) {
				...
				if( inputElement instanceof Document ) {
					...
					Node node = (Node) XPathUtils.evaluateXPathExpression(
								expr, SaMasterPage.this.saFormPage.getDocument(), XPathConstants.NODE );

					if( node != null )
						result = new Object[] { node };
				}
			...
			public Object[] getChildren( Object parentElement ) {
				...
				if( parentElement instanceof Element ) {
					String name = DomUtils.getNodeName((Element) parentElement);
					if( "service-assembly".equals( name )) {
						String expr = "*[local-name()='service-unit']";
						NodeList pc = (NodeList) XPathUtils.evaluateXPathExpression(
									expr, (Element) parentElement, XPathConstants.NODESET );

						result = new Object[ pc.getLength()];
			...
			public void handleEvent( Event event ) {
				...
				while( it.hasNext()) {
					...
					if( o instanceof Element ) {
						...
						SaMasterPage.this.saFormPage.startTransaction();
						DomUtils.removeElement((Element) o);
						SaMasterPage.this.saFormPage.stopTransaction();
	...
	private void addServiceUnit() {
		...
		if( dlg.open() == Window.OK ) {
			...
			String expr = "/*[local-name()='jbi']/*[local-name()='service-assembly']";
			Node saNode = (Node) XPathUtils.evaluateXPathExpression(
						expr, this.saFormPage.getDocument(), XPathConstants.NODE );

			if( saNode == null || saNode.getNodeType() != Node.ELEMENT_NODE )
				return;

			this.saFormPage.startTransaction();
			Element saElt = (Element) saNode;
			Element newElt = StructuredModelHelper.getChildElementToInsert(
						saElt,
						"service-unit",
						StructuredModelHelper.INSERTION_APPEND ).getElement();

			if( newElt != null ) {
				saElt.appendChild( newElt );
				...
				this.saFormPage.stopTransaction();
	...
	private void updateViewerInput() {
		this.viewer.setInput( this.saFormPage.getDocument());
	}
